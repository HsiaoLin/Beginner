...
<div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="user/user/list" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
...
function add(){
	...
	diag.Drag=true;
	diag.Title ="新增";
	diag.URL = '<%=basePath%>user/user/goAdd';
	diag.Width = 450;
	diag.Height = 355;
...
function edit(Id){
	...
	diag.Drag=true;
	diag.Title ="编辑";
	diag.URL = '<%=basePath%>user/user/goEdit?USER_ID='+Id;
	diag.Width = 450;
	diag.Height = 355;
	diag.CancelEvent = function(){ //关闭事件
		if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
//导出Excel
function toExcel(){
	window.location.href='<%=basePath%>user/user/excel';
}
</script>
<script type="text/javascript">
$(function() {
