...
<div id="zhongxin">
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<form action="job/goList" method="post" name="Form" id="Form">
							...
							<div class="widget-main  form-horizontal">
								<table class="table table-bordered">
									<tr>
										...
										<td class="td2">
											<select class="chzn-select" name="jobStatus"
												id="jobStatus" data-placeholder="任务状态"
												style="vertical-align:top;width: 120px;">
												<option value="">请选择</option>
												<option <c:if test="${not empty pd and pd.jobStatus=='NONE'}">selected</c:if> value="NONE">未知</option>
												<option <c:if test="${not empty pd and pd.jobStatus=='NORMAL'}">selected</c:if> value="NORMAL">正常运行</option>
												<option <c:if test="${not empty pd and pd.jobStatus=='PAUSED'}">selected</c:if> value="PAUSED">暂停状态</option>
												<option <c:if test="${not empty pd and pd.jobStatus=='COMPLETE'}">selected</c:if> value="COMPLETE">完成状态</option>
												<option <c:if test="${not empty pd and pd.jobStatus=='ERROR'}">selected</c:if> value="ERROR">错误状态</option>
												<option <c:if test="${not empty pd and pd.jobStatus=='BLOCKED'}">selected</c:if> value="BLOCKED">锁定状态</option>
											</select>
										</td>
									...
									<tr>
										<td style="vertical-align:middle;text-align: center;" colspan="100">
											<button class="btn btn-success" onclick="search();"><i id="nav-search-icon" class="icon-search"></i>搜索</button>
											<a class="btn btn-danger" onclick="resetForm();" title="清空"><i id="nav-search-icon" class="icon-trash"></i>清空</a>
										</td>
									</tr>
								</table>
							</div>
