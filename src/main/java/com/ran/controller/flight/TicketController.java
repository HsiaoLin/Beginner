 ...
 */
@Controller
@RequestMapping(value="/flight/ticket")
public class TicketController extends BaseController {
	
	String menuUrl = "flight/ticket/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, "新增Ticket");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("TICKET_ID", this.get32UUID());	//主键
	...
	public ModelAndView list(Page page){
		...
		try{
			...
			mv.setViewName("flight/ticket/ticket_list");
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增Ticket页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("flight/ticket/ticket_edit");
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出Ticket到excel");
